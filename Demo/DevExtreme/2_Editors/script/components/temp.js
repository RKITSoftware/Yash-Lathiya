...
export default function addSelectBox() {
  ...
  const selectBoxWidget = itemsSelectBox
    .find(".dx-field-value")
    .dxSelectBox({
      items: [...lstProduct],
      ...
      buttons: [
        "clear",
        {
          ...
          options: {
            icon: "arrowup",
            onClick: function (e) {
              let highPriceItem = selectBoxWidget.option("items").reduce(
                (acc, item) => {
  ...
  const dsSelectBoxWidget = dsSelectBox
    .find(".dx-field-value")
    .dxSelectBox({
      grouped: true,
      groupTemplate(data) {
        return $(
          `<div class='custom-icon'><span class='dx-icon-box icon'></span> ${data.key}</div>`
        );
      },
      itemTemplate(data) {
      ...
      fieldTemplate(data, container) {
        let result;
        if (data == null) {
          result = $("<div>").dxTextBox({
            placeholder: "Select Product",
            readOnly: true,
          });
        } else {
          result = $(
            `<div class='custom-item'><div class="custom-item-image"><img alt='Product name' src='${
              data ? data.imgSrc : ""
  ...
  let instance1 = sESelectBoxFieldWrapper
    .find(".dx-field-value")
    .dxSelectBox({
      acceptCustomValue: true,
      dataSource: [
      ...
      onCustomItemCreating(e) {
        ...
        let extItem = companyDataStore
          .items()
          .filter((company) => company.name === nameLocation[0])?.[0];
        if (extItem) {
